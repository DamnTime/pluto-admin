...
# 创建 app 目录
RUN mkdir -p /usr/src/node-app/pluto-admin

# 设置工作目录
WORKDIR /usr/src/node-app/pluto-admin
...
COPY package.json /usr/src/node-app/pluto-admin/package.json
...
# 拷贝所有源代码到工作目录
COPY . /usr/src/node-app/pluto-admin
